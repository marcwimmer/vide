FROM ubuntu:16.04
RUN apt-get update
RUN apt-get install -y htop build-essential rsync git python cmake libncurses5-dev python-dev ruby-dev nodejs npm libpython-dev build-essential cmake ctags apt libtool libtool-bin autoconf automake cmake g++ pkg-config unzip wget pkg-config liblzma-dev libpcre3-dev silversearcher-ag python-pip curl golang-go
